...
| `TTS_BACKEND` | `piper` | `piper`（CPU・英語のみ）/ `xtts`（GPU・多言語）。**日本語(`lang=ja`)は `xtts` が必要**（Piper に日本語ボイスが無く、ja の音声生成は失敗する）。フロントは `GET /api/tts/capabilities` を見て、現在のバックエンドで音声化できない言語では「音声を生成」を無効化する（#89） |
