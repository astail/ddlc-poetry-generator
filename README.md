...
詳細は [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md) / [docs/SPEC.md](./docs/SPEC.md) / [docs/API.md](./docs/API.md)（API リファレンス）を参照。
...
| `POEM_MAX_RETRIES` / `POEM_TIMEOUT` / `POEM_PARSE_RETRIES` | `3` / `60` / `2` | Claude 呼び出しの SDK リトライ / タイムアウト秒 / JSON パース再試行回数 |
...
| `SD_MODEL_TYPE` | `sd15` | 既定チェックポイントの種別（`sd15` / `sdxl`）。解像度・ワークフローの選択に使う |
