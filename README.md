...
- 🔊 **読み上げ**: 英語=Piper（CPU・キャラ別ボイス）/ XTTS（GPU・任意）、日本語=VOICEVOX（アニメ調キャラボイス・任意）
...
| `VOICEVOX_URL` | （未設定） | **日本語(`lang=ja`)** の読み上げに使う VOICEVOX エンジンの URL（例: `http://voicevox:50021`）。`docker compose --profile voicevox up -d` で起動して設定すると日本語音声が有効化される。未設定の間はフロントが `GET /api/tts/capabilities` を見て「音声を生成（日本語）」を無効化する（#89） |
