...
### MVP（M1〜M3）
- キャラ選択（Sayori / Natsuki / Yuri / Monika）＋お題（任意）入力（「ランダム / 全員」は未実装・将来検討）
...
| **Natsuki** | 短く可愛くツン。勢いのある一言だけでなく、素朴で童謡的な寓話（動物の「〜できる」列挙、変わった趣味の友達の話）も。子どもっぽい表面の裏に「見下されること」への反発。 | ポップ、パステル、お菓子・かわいい小物 |
...
- 進捗取得は専用エンドポイント（旧仕様の `GET /api/poems/{id}/status` SSE）ではなく、フロントが `GET /api/poems/{id}` を約2秒間隔でポーリングして画像・音声の `status` を確認する（簡略化のため SSE は未実装）
